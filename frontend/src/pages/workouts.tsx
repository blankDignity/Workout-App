...
const workoutCardContents = [
  {
    level: "Intermediate",
    category: "Strength",
    label: "Chest & Triceps Blast",
    duration: 45,
    calories: 420,
    exercises: 8
  },
...
export default function Workouts() {
  ...
  return (
      <div className={"w-full ml-70 my-10 mr-10"}>
        <div className={"w-full flex justify-between items-center"}>
          <div className={"font-bold text-5xl"}>
            My Workouts
          </div>
          <div className={"flex font-bold items-center gap-2 bg-[#ff4757] p-3 rounded-2xl px-5"}>
          ...
          {categories.map((category) => {
            ...
            return (
                <div key={category} onClick={() => setActiveCategory(category)}
                     className={`cursor-pointer border border-zinc-700 px-6 py-2 rounded-2xl font-medium transition-all duration-200 ${
                         isActive ? 'bg-[#ff4757] text-white shadow-md' : 'bg-[#16161d] text-white hover:bg-[#1e1e26]'}`}>
                  {category}
                </div>
            );
          })}
        ...
        <div className={"grid grid-cols-2 gap-4"}>
          {workoutCardContents.map((workoutCard) => (
